...
public class TeacherController {
    ...
    public Integer add(@RequestBody TeacherDTO entity) {
        entity.setTeaId(IdUtil.getNewId());
        ...
        for (int i = 0; i < entity.getCourseIds().length; i++) {
            ...
            courseTeaEntity.setId(IdUtil.getNewId());
    ...
    public TeacherDTO getAdmin(@RequestParam(value = "teaId") Long teaId) {
        ...
        Long[] arr = new Long[courseTeaEntities.size()];
     ...
     */
    @RequestMapping("/exit")
    public Boolean exit(@RequestParam(value = "teaId") Long teaId) {
        HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest();
        HttpSession session = request.getSession();
        //学生id
        Long stuId = Long.parseLong(session.getAttribute("loginUserId").toString());
